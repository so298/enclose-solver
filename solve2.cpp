...
struct DynamicBitset {
    ...
    inline void for_each_set_bit(F&& f) const {
        for (size_t wi = 0; wi < w.size(); wi++) {
            uint64_t x = w[wi];
            while (x) {
                uint64_t lsb = x & (~x + 1ULL); // x & -x but unsigned-safe
...
struct FlowTemplate {
    int n;
    vector<vector<int>> adj;     // outgoing edge indices
    vector<vector<int>> in_adj;  // incoming edge indices
...
static inline bool is_open_cell(char ch) {
    return ch == '.' || ch == 'H';
}

SolveResult solve(int k, const vector<string>& grid) {
...
void print_ans(int area, const vector<pair<int,int>>& walls, const vector<string>& grid_original) {
